...
솔직히 말하면 나도 그랬다. 기능 구현이 급하니까 일단 코드부터 짜고, 테스트는 여유 있을 때... 라고 생각했다. 그 "여유 있을 때"는 영원히 오지 않았고, 결국 버그가 터질 때가 되서야 디버깅하고 테스트를 진행하며 코드 수정하는 나를 발견했다.

이번에 회원 기능(회원가입, 내 정보 조회, 비밀번호 수정)을 TDD로 구현해보면서, 왜 테스트를 먼저 짜야 하는지 몸소 깨달았다. 테스트 없이 코드를 짜는 건, 정말 **눈 감고 운전하는 것**과 다름없었다.
...
도메인 테스트에서 실제 BCrypt를 쓰면 느리고, 결과값 예측이 어렵다. 그래서 Stub을 만들었다.
...
이렇게 하면 `encode("password123")`의 결과가 항상 `"encoded_password123"`이다. 테스트에서 예측 가능하고, 빠르다.
...
**결론:** 현재 규모에서는 과할 수 있지만, 테스트 작성이 편해지고 의존성이 명확해지는 장점이 있어서 채택했다. "지금 당장"보다 "앞으로의 확장성"을 고려한 선택이다.
...
데이터는 엔티티에, 로직은 서비스에 흩어진다. 엔티티에 로직이 있으면 **관련 있는 것들이 한 곳에 모인다**.
...
같은 검증을 여러 곳에서 하면 규칙 바뀔 때 다 고쳐야 한다. **도메인 규칙은 도메인 테스트에서 보장**하면 된다.
...
단위 테스트를 촘촘히 깔고, 통합 테스트로 연결 확인하고, E2E로 전체 검증. 이 구조가 유지보수에 좋다.
